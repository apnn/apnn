...
import TestGenericMR.TestGenericMapTerms;
 ...
 * @author jeroen
 */
public class LSHCosMap extends TestGenericMapTerms {

    public static final Log log = new Log(LSHCos.LSHCosMap.class);
    AnnLSHCos index;
    ...
    public void map(Integer key, Pair value, Context context) throws IOException, InterruptedException {
        ...
        for (Map.Entry<Document, long[]> query : index.queryIterator()) {
            ...
            for (Candidate candidate : topKSourceDocuments) {
                candidate.query = suspiciousDocument.docid;
