 ...
 * @author jeroen
 */
public class LSHCosNYTMap extends Mapper<String, String, Text, Candidate> {

    public static final Log log = new Log(LSHCos.LSHCosNYTMap.class);
    protected Conf conf;
    protected DocumentReader documentreader;
    protected AnnLSHCos index;
    protected int topk;
    ...
    public void map(String sourcefilename, String queryfilename, Context context) throws IOException, InterruptedException {
        ...
        if (index == null) {
            try {
                index = new AnnLSHCos(getComparator(), getConf());
            } catch (ClassNotFoundException e) {
                log.fatalexception(e, "new AnnLSHCos index");
            }
            Datafile sourceDatafile = new Datafile(conf, sourcefilename);
            sourceDocuments = documentreader.readDocuments(sourceDatafile);
        }
        // read all source documents and add to AnnIndex

        if (sourcefilename.equals(queryfilename)) {
            index.set(sourceDocuments);
            ...
            ArrayList<Document> queryDocuments = documentreader.readDocuments(queryDatafile);
            index.set(sourceDocuments, queryDocuments);

            for (Document query : queryDocuments) {
                // retrieve the k most similar source documents from the index
